...
LPPROCESS_INFORMATION Inject(const char *processName, int pid, const char *dllpath, _Bool verbose)
{
	...
	THROWIF(dwAttrib == INVALID_FILE_ATTRIBUTES || dwAttrib & FILE_ATTRIBUTE_DIRECTORY, GetLastError(), verbose);
	...
	LPVOID pAlloc = NULL;
	if (hndProc)
		pAlloc = VirtualAllocEx(hndProc, NULL, (strlen(absolutePath) + 1) * sizeof(char), MEM_RESERVE | MEM_COMMIT,
	...
	if (hndProc && pAlloc)
		THROWIF(!WriteProcessMemory(hndProc, pAlloc, absolutePath, (strlen(absolutePath) + 1) * sizeof(char), NULL),
	...
	DWORD threadID = 0;
	if (hndProc && pAlloc)
		thread = CreateRemoteThread(hndProc, NULL, 0, (LPTHREAD_START_ROUTINE)LoadLibrary, pAlloc, 0, &threadID);
...
LPPROCESS_INFORMATION Eject(const char *processName, int pid, const char *dllpath, _Bool waitForExit, _Bool verbose)
{
	...
	for (BOOL bMoreMods = Module32First(snapshot, &ModEntry); bMoreMods; bMoreMods = Module32Next(snapshot, &ModEntry))
	{
	...
	// Open target process
	HANDLE hndProc =
		OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_CREATE_THREAD | PROCESS_VM_OPERATION, FALSE, procID);
	...
	DWORD threadID = 0;
	if (hndProc)
		thread = CreateRemoteThread(hndProc, NULL, 0, (LPTHREAD_START_ROUTINE)FreeLibrary, ModEntry.modBaseAddr, 0,
			&threadID);
